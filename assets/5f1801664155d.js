...
/* Copyright (c) 2020 Elucidat Ltd - All rights reserved */
console.log("Elucidat.com Rapid Authoring (v.#build_version#) built with love on 2020-07-22 09:07:06");
/* 
notes - to add:
    1.2
        cmi.core.student_id
		...
		if ($.fn.popover && $.fn.modal.Constructor) {
			...
			// pause any video / audio on popup hide
		  	$(document).on('hide.bs.modal', '.modal', function (e) {
                var $this = $( this );
                $this.find('div.video_player').video('pause');
    ...
    if(this.statement_queue.length > 0 ){
        ...
        for(var i = 0; i <this.endpoints.length; i++){
            ...
            if(call.statements.length || !window.done_first_error){
                this._do_ajax_call (async, call.endpoint, call.authentication, call.statements, function(data){
                    for (var x = 0; x < call.callbacks.length; x++) {
                        if(typeof call.callbacks[x] === 'function'){
                            call.callbacks[x](data); 
                        }
                        //each statement can have its own callback so if its been batched loop through them all
                    }
                });
        ...
        $('div#pew').off('section_complete answered').on('section_complete answered', function (e, result) {
            ...
            for (var i=0;i<current_page.completable_sections.scored.length;i++) {
                if (e.type == 'answered' && current_page.completable_sections.scored[i].target_id === e.target.id) {
                    ...
                    if(result.interaction_id && result.interaction_type !== 'likert' 
                    && result.interaction_type !== 'swipe' && $(e.target).prop('tagName') !== 'TR'
                    && result.interaction_type !== 'long-fill-in' && $(e.target).prop('tagName') !== 'TR') {
...
Elucidat.prototype._open_page = function ( page_object ) {
    ...
    if (useGoogleAnalytics === 'true' || useGoogleAnalytics === null) {
        ...
        ga(function() {
            var gaTrackersList = context._ga_getter();
            if (gaTrackersList && gaTrackersList.length) {
                for (var i = 0; i < gaTrackersList.length; i++) {
                    ga.remove(gaTrackersList[i]);
                }
            }
        });
    }
    ...
    if (!$pew.length) {
        ...
        setTimeout(function () {
            ...
            var $load_anchor = $('<a id="load_anchor" name="load_anchor" tabIndex="-1" title="You are at the top of page" />');//++'
            ...
            return this.each(function() {
                ...
                $form.find('a.save_button, button.save_button').each(function () {
                    ...
                    if ( !$form.hasClass('answered') ) {
                        $button.addClass('fake_save_button--disabled')
                    ...
                    $button.click(function(e){
                        ...
                        if( $button.hasClass('fake_save_button--disabled') ) {
                            return;
                        }
        ...
        update_form_status: function(options) {
            ...
            return this.each(function() {
                ...
                if ( options.is_answered ) {
                    ...
                    $save_button.removeClass('fake_save_button--disabled');
                    ...
                    $save_button.addClass('fake_save_button--disabled')
                        ...
                        .each(function () {
                            ...
                            $('body').one('elucidat.page.ready', function () {
                                ...
                                var draggable_options = {
                                    scroll: !interactionSettings.useDropboxDrawer, // prevents the page from scrolling while dragging
                                    scrollSpeed: 5,
                                    ...
                                    containment: dragContainer, // prevents item from being dragged outside of the container element
                                    scope: $form.attr('id'),
                            ...
                            $dropboxDrawer
                                .on('droparea_reveal', function(e, dropper) {
                                    $(this)
                                        ...
                                        .parents('body').addClass('e-modal-open')

                                    dropper
                                        .css('z-index', 110)
                                        .parents().css('z-index', 'auto')
                                })
                                .on('droparea_hide', function(e, dropper) {
                                    ...
                                    setTimeout(function () {
                                        ...
                                        $droppers.not('.e-dropped')[0].focus()
                                    }, 300);
                                    ...
                                    $dropboxes
                                        ...
                                        .keydown(function (e) {
                                            if (e.key === 'Escape') {
                                                ...
                                                if ($(this).attr('id') === $focusedDropper.attr('href').replace('#', '')) {
        ...
        free_text_update_form: function(options) {
            ...
            return this.each(function() {
                ...
                $freeText.each(function() {
                    ...
                    var count = options.maxlength - $(this).val().length;
            ...
            return this.each(function() {
                ...
                var set_interactions = function() {
                    ...
                    $interactions.each(function(i) {
                        ...
                        .click(function (e) {
                            e.preventDefault();
                            $interactions.filter(':not(.e-is-answered):first')[0].focus()
                            return false;
                        })
                        ...
                        // find correct answer
                        // if correct answer is not defined then default to '1' (false)
                ...
                var submit_form = function( $interaction, is_scored ) {
                    ...
                    if ( data_swipe ) {
                        user_answer.push( parseInt($interaction.attr('data-swipe')) );
                    }
                    ...
                    if ( is_scored ) {
                        ...
                        var answered_correct = $interaction.attr('data-status') == data_swipe ? true : false;
                    ...
                    // if visible already
                        ...
                        if (audio_object[0] == 'audio') {
                            ...
                            if (audio_url) {
                                ...
                                var html = '<audio class="e-mejs-player" id="audio'+ (audio_id) +'" src="'+base_url +'.mp4" preload="none">'+//'
...
(function($){
    $('.audio').on('controlsresize', function(){
        //Fix for iOS mediaelements issue where the audio players seek bar resizes to 1px
        var $this = $(this);
        ...
        if( $time_rail.css('width') === '1px'){
            $controls.children('div').not('.mejs-time-rail').each(function(){
                var $div = $(this);
                if($div.is(':visible')){
                    width = width - $div.outerWidth(true);
                }
            });
            width = width - parseInt($time_rail.css('marginLeft')) - parseInt($time_rail.css('marginRight')); //to compensate for margins
        ...
        collapse_complete: function(options) {
            ...
            return this.each(function() {
                ...
                $collapse.on('shown', function () {
                    ...
                    if ( !$collapse.find('.media.video.e-has-video').length ) {
                ...
                if (options === 'destroy') {
                    ...
                    if ($link.attr('data-tooltip')) {
                        ...
                        $link.on('mouseover focus', function () {
                            // clear active popovers
                            for (var i = 0; i < popovers.length; i++) {
                                // only clear popovers if they are shown - stops
                                // accessibility bugs
                                if (popovers[i].get(0) !== $link.get(0) && popovers[i].data('popover').tip().hasClass('in') ) {
                                    popovers[i].popover('hide');
                                }
                            }
                            
                            // if the popover is not already open... open it
                            if ( !$link.data('popover').tip().hasClass('in') ) {
                                $link.popover('show');
                            }
                        });
...
(function ($) {
    setTimeout(function () {
        // var pluginV = 43;
        // console.log(
        //     "%c// ios_iframe_fix => version " + pluginV + ""
        //     , "color: deeppink; font-weight: bold;"
        // );
        
        if ( (window.self !== window.parent && $('html').hasClass('ios')) ) {
            // alert('plugin ' + pluginV);
            
            // console.log(
            //     "%c// ios_iframe_fix => plugin activated"
            ...
            $body.find('.project__footer.e-pos--fixed').css('position', 'absolute');
            ...
            var resizeContent = function() {
                ...
                if ((old_w !== w || count > 20) && !done) {
                    // console.log(
                    //     "%c// ios_iframe_fix: resize: w:" + w + ", h:" + h
                    //     , "background: #222; color: #11bbdd; font-weight: bold;"
                    // );
                    $both.show();
                    $both.css({
                        'height': h,
            ...
            // console.log(
            //     "%c// ios_iframe_fix => plugin NOT activated \n" +
        ...
        accessibility_fixes: function(){
            ...
            return this.each(function () {
                        ...
                        if($closest_a.attr('data-dismiss') === 'dropdown' || $closest_a.attr('data-toggle') === 'modal' || $e.hasClass('project__menu') || $e.hasClass('menu__wrap')){
                            ...
                            if($buttonThatOpenedMenu){
                                if($buttonThatOpenedMenu.attr('data-target')){
                                    setTimeout(function(){$buttonThatOpenedMenu.focus();}, 50);
                                }
                            }else{
                                setTimeout(function(){$("#load_anchor").focus();}, 50);
                            }
                        }
                    }
                ...
                function fixRadioBtns($form){
                    ...
                    var $group = $('<div></div>');
                    $group.attr('role', 'radiogroup').attr('labelledby', '#' + $q_header.attr('id'));
                    $form.append($group);
        ...
        fix_links: function( context ) {
            ...
            return this.each(function () {
                ...
                function onClickInternalLink(ev) {
                    ...
                    if($target.length) {
                        var topPos = $target.offset().top - $('.project__header').data('fixed-header-height');
...
ga('create', 'UA-40048511-3', 'auto');var gac='';if(gac.length){ga('create',gac,'auto',{'name':'c'});}
